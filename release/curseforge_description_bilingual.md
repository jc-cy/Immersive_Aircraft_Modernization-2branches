# Immersive_Aircraft_Modernization / 沉浸飞机：现代化
...
沉浸飞机：现代化为沉浸飞机增加“超频巡导模块”，为飞机和可悬停类载具提供全自动航线导航。
...
- 路线预加载提供不加载、直线飞行和 L 形飞行三种模式。
- 直线飞行保持三格宽路线；L 形飞行在长距离斜线航段使用单格宽路径。
- 预加载可降低高速导航时的地形加载压力。

### HUD 信息

- 显示速度、燃料数量、燃料剩余时间和飞行剩余时间估算。
- 飞行剩余时间按水平航线距离和当前速度估算；L 形航段按实际两轴折线路径估算。
- 支持普通燃料和液体燃料容器。
- 显示航线名、起点、上站、下站和终点。
...
- `,` 打开航线设置。
- `.` 启动、暂停、继续或退出自动导航。
- 支持驾驶位专属编辑与导航控制、乘客只读信息、撞击保护、精细降落控制和成就进度。

必需前置：沉浸飞机（Immersive Aircraft）。
适用于 Minecraft 1.20.1 Forge。
协议：All Rights Reserved (ARR)。完整条款请查看随附的 `LICENSE` 与 `DISCLAIMER.md`。
