...
public final class ClientPacketHandlers {
    ...
    private static void decodeCruiseChunk(CruiseRouteChunkPacket packet, long session) {
        ...
        try {
            ...
            decoded = decodeChunkPacket(payload);
    ...
    /**
     * 1.21 removed the {@code ClientboundLevelChunkWithLightPacket(FriendlyByteBuf)} constructor; the
     * chunk packet is decoded through its stream codec, which needs the client level's registry access.
     */
    private static ClientboundLevelChunkWithLightPacket decodeChunkPacket(byte[] payload) {
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.level == null) {
            throw new IllegalStateException("Client level is unavailable for cruise chunk decoding");
        }
        RegistryFriendlyByteBuf buffer = new RegistryFriendlyByteBuf(Unpooled.wrappedBuffer(payload),
    ...
    private static boolean reanchorAircraftCopy(VehicleEntity vehicle, double x, double y, double z,
        ...
        vehicle.lerpTo(x, y, z, vehicle.getYRot(), vehicle.getXRot(), 0);
    ...
    private static void sendRideResync(String reason, Entity root, boolean hardReset) {
        ...
        CruiseNetwork.sendToServer(new RequestCruiseRideResyncPacket(rootId, hardReset, clientControls));
    }
