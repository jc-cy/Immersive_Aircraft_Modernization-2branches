...
public final class CruiseHud {
    ...
    public static void render(GuiGraphics graphics, DeltaTracker deltaTracker) {
        ...
        renderHud(graphics, minecraft.font, vehicle, access, route, graphics.guiWidth(), graphics.guiHeight());
    }
    ...
    public static void renderHudAt(GuiGraphics graphics, Font font, VehicleEntity vehicle,
    ...
    public static void renderHudAt(GuiGraphics graphics, Font font, VehicleEntity vehicle, boolean boosting,
        ...
        if (boosting) {
            CruiseModuleData.setBoosting(moduleIcon, true);
        }
        ...
        graphics.drawString(font, "速度 " + format(speed) + " 格/秒", textX, y + 2, 0xFFFFFFFF, true);
        graphics.drawString(font, "燃料剩余 " + fuelInfo.amountText(), textX, y + 22, 0xFFFFFFFF, true);
        graphics.drawString(font, "剩余时间 " + formatTime(fuelInfo.remainingTicks()), x, y + 42, 0xFFFFFFFF, true);
        int detailsY = y + 54;
        String routeName = route.getSelectedRouteDisplayName();
        if (!routeName.isBlank()) {
            graphics.drawString(font, fitText(font, routeName, CruiseHudSettings.WIDTH), x, detailsY, 0xFFFFE28A, true);
            detailsY += 10;
        }
        graphics.drawString(font, "起点 " + formatPoint(route.getStartPoint()), x, detailsY, 0xFFD8D8D8, true);
        graphics.drawString(font, "上站 " + formatPoint(previousPoint(route)), x, detailsY + 10, 0xFFD8D8D8, true);
        graphics.drawString(font, "下站 " + formatPoint(nextPoint(route)), x, detailsY + 20, 0xFFD8D8D8, true);
        graphics.drawString(font, "终点 " + formatPoint(route.getFinalTarget()), x, detailsY + 30, 0xFFD8D8D8, true);
    }
